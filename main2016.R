...
pitchers_final <- pitchers %>% filter()  mutate(wOBA = calcWOBA(SEASON,AB,X1B,X2B,X3B,HR,UBB,HBP,SF)) %>% 
  ...
  mutate(minPA=ifelse(totPA <200,0,0.7*totPA), maxPAvsL=ifelse(totPA >= 502,160,pmin(LHB_PA*1.25,160)), maxPAvsR=ifelse(totPA >= 502, 545, pmin(RHB_PA*1.25,545))) %>% 
  mutate(LHB_Runs = maxPAvsL*LHB_wRAA, RHB_Runs = maxPAvsR*RHB_wRAA, Total_Runs = LHB_Runs + RHB_Runs) %>% 
  arrange(-Total_Runs)
